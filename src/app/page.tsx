...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const fetchTopReviewers = async () => {
      try {
        const res = await fetch("/api/users/top-reviewers");
        if (res.ok) {
  ...
  return (
    <motion.div variants={containerVariants} initial="hidden" animate="visible">
      ...
      <motion.div
        ...
        className="w-full min-h-[calc(100vh-4rem)] md:min-h-[calc(90vh-4rem)] lg:min-h-[calc(70vh-4rem)] flex items-center justify-center px-6 text-black overflow-hidden"
      >
      ...
      {/* LEADERBOARD / TOP REVIEWERS SECTION */}
      <motion.div
        initial={{ opacity: 0 }}
        whileInView={{ opacity: 1 }}
        viewport={{ once: true, amount: 0.3 }}
        className="w-full border-t-4 border-black py-12 md:py-16 px-6 text-black flex flex-col items-center justify-center bg-[#A7F3D0]"
      >
        <div className="text-center mb-8 md:mb-12 space-y-3">
          <h2 className="text-3xl md:text-5xl font-black text-white [-webkit-text-stroke:5px_black] [paint-order:stroke_fill] tracking-tight">
        ...
        <div className="w-full max-w-3xl flex flex-col gap-4">
          {loading ? (
            /* Simple Loading Skeleton με Neo-brutalist αισθητική */
            <div className="text-center font-black text-xl animate-pulse py-6">
              LOADING TOP REVIEWERS...
            </div>
            ...
            topReviewers.map((reviewer, idx) => (
              ...
              >
                ...
                <div className="flex items-center gap-4 w-full sm:w-auto justify-start">
                  ...
                  <div
                    style={{ backgroundColor: reviewer.color || "#9333EA" }}
                    className="w-12 h-12 rounded-xl border-2 border-black flex items-center justify-center text-2xl shadow-[2px_2px_0px_0px_black] overflow-hidden relative"
                  >
                  ...
                  <span className="font-black text-xl tracking-tight text-black truncate max-w-[100px] sm:max-w-[100%]">
                    {reviewer.name}
                  </span>
                </div>
