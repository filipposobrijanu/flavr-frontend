"use client";

import Link from "next/link";

export default function AboutPage() {
  return (
    <div className="min-h-[calc(80vh-4rem)] p-6 md:p-12 text-black">
      <title>About | Flavr</title>
      <div className="max-w-4xl mx-auto space-y-12">
        {/* Κουμπί Επιστροφής */}
        <Link
          href="/"
          className="inline-block font-black inline-flex gap-2 text-md uppercase tracking-wider border-2 border-black bg-gray-200 px-4 py-2 rounded-xl shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[1px] hover:translate-y-[1px] hover:shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] transition-all"
        >
          Back to Home
        </Link>

        {/* 🍕 Κεντρικό Hero Section */}
        <div
          style={{ backgroundColor: "#FE2120" }}
          className="relative border-4 border-black p-6 rounded-2xl shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]"
        >
          <div className="absolute right-4 bottom-0 text-4xl opacity-20 pointer-events-none font-black select-none hidden sm:block">
            FOOD
          </div>
          <h1 className="text-3xl md:text-5xl font-black uppercase tracking-tight text-black">
            What is Flavr?
          </h1>
        </div>

        {/* 🎯 Η Φιλοσοφία μας (Problem & Solution) */}
        <div className="bg-white p-6 md:p-8 border-2 border-black rounded-2xl shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] space-y-4">
          <h2 className="text-2xl md:text-3xl font-black uppercase tracking-tight text-black">
            Why did we build Flavr?
          </h2>
          <div className="text-sm md:text-base font-medium text-gray-800 space-y-4 border-t-2 border-black pt-4 leading-relaxed">
            <p>
              We are tired of the platforms where a restaurant has{" "}
              <span className="font-black bg-yellow-300 px-1">5.0 stars</span>{" "}
              because only 2 friends of the owner voted, or{" "}
              <span className="font-black bg-red-300 px-1">1.0 star</span>{" "}
              because a customer waited 5 minutes longer for an order.
            </p>
            <p>
              Flavr was born to provide the solution by bringing to the fore the{" "}
              <strong className="font-black text-black uppercase">
                objectivity and mathematics
              </strong>
              , offering a fair and transparent look at the gastronomic map.
            </p>
          </div>
        </div>

        {/* 🛠️ Core Features Grid */}
        <div className="space-y-4">
          <h3 className="text-2xl font-black text-white uppercase tracking-tight [-webkit-text-stroke:4px_black] [paint-order:stroke_fill]">
            How does it work?
          </h3>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* Feature 1 */}
            <div className="bg-white p-6 border-2 border-black rounded-2xl shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] space-y-2">
              <h4 className="text-xl font-black uppercase tracking-tight">
                Global Bayesian Score
              </h4>
              <p className="text-sm text-gray-700 font-medium leading-relaxed">
                We don't use a simple average. The **Bayesian Average**
                algorithm takes into account the total number of reviews across
                the platform. This ensures that the ratings are stable, valid
                and not easily distorted by individual spam reviews.
              </p>
            </div>

            {/* Feature 2 */}
            <div className="bg-white p-6 border-2 border-black rounded-2xl shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] space-y-2">
              <h4 className="text-xl font-black uppercase tracking-tight">
                Multi-Criteria Ratings
              </h4>
              <p className="text-sm text-gray-700 font-medium leading-relaxed">
                A good meal is not just about the plate. At Flavr, you can rate
                the Food, Service, Atmosphere and Value for Money (VFM)
                separately, giving you a complete picture of your experience.
              </p>
            </div>

            {/* Feature 3 */}
            <div className="bg-white p-6 border-2 border-black rounded-2xl shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] space-y-2">
              <h4 className="text-xl font-black uppercase tracking-tight">
                Verified Reviewers
              </h4>
              <p className="text-sm text-gray-700 font-medium leading-relaxed">
                We separate roles on the platform. Only registered users with
                the **Reviewer** role can submit live reviews, ensuring the
                validity of the community.
              </p>
            </div>

            {/* Feature 4 */}
            <div className="bg-white p-6 border-2 border-black rounded-2xl shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] space-y-2">
              <h4 className="text-xl font-black uppercase tracking-tight">
                Zero Sponsored Hype
              </h4>
              <p className="text-sm text-gray-700 font-medium leading-relaxed">
                We don’t accept payments from restaurant owners to hide bad
                reviews, erase mistakes, or artificially boost ratings. What you
                see is exactly what the community experienced—raw, honest, and
                completely unedited.
              </p>
            </div>
          </div>
        </div>

        {/* 💻 Tech Stack Sticker */}
        <div className="bg-yellow-400 border-2 border-black p-6 rounded-2xl shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] text-center space-y-2">
          <p className="text-xs font-black uppercase tracking-wider text-black">
            Built With Modern Full-Stack Tech
          </p>
          <p className="text-sm md:text-base font-black text-black tracking-wide uppercase">
            React • Next.js • Tailwind CSS • Prisma • SQLite
          </p>
        </div>
      </div>
    </div>
  );
}
